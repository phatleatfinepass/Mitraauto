...
with tire_source as (
  select
    ...
    ps.best_image_url as api_best_image_url,
    ps.hero_image_url as api_hero_image_url,
    ps.best_image_alt,
...
effective_rows as (
  select
    ...
    coalesce(
      nullif(btrim(ts.cms_hero_image_url), ''),
      case
        when jsonb_typeof(coalesce(ts.cms_gallery, '[]'::jsonb)) = 'array'
          then nullif(btrim(ts.cms_gallery->>0), '')
        else null
      end,
      nullif(btrim(ts.api_hero_image_url), ''),
      nullif(btrim(ts.api_best_image_url), '')
    ) as best_image_url,
    coalesce(
      nullif(btrim(ts.cms_hero_image_url), ''),
      case
