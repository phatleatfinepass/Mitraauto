...
create or replace function public.catalog_v3_nullif_blank(p_text text)
returns text
language sql
immutable
as $$
...
create or replace function public.catalog_v3_safe_numeric(p_text text)
returns numeric
language sql
immutable
as $$
  select case
    when public.catalog_v3_nullif_blank(p_text) is null then null
    when regexp_replace(replace(trim(p_text), ',', '.'), '\s+', '', 'g') ~ '^-?[0-9]+(\.[0-9]+)?$'
      then regexp_replace(replace(trim(p_text), ',', '.'), '\s+', '', 'g')::numeric
    else null
  end;
$$;
...
revoke all on function public.cms_count_tires_admin_v1(text, boolean, boolean, text) from public;
grant execute on function public.cms_count_tires_admin_v1(text, boolean, boolean, text) to authenticated;
