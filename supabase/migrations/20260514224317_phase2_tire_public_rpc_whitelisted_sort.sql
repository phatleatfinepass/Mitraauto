-- Phase 2: make public tire listing product-ready and remove ORDER BY CASE.
-- The sort expression is chosen only from server-owned constants so client input
-- never becomes SQL. This lets Postgres use the dedicated storefront indexes.
...
begin
  ...
  return query execute
    ...
    order by ' || v_order_by || '
    limit greatest(coalesce($15, 24), 1)
    offset greatest(coalesce($16, 0), 0)'
    using
      p_search,
      p_brands,
      p_width,
...
create index if not exists webshop_items_tire_ready_stock_idx
...
revoke all on function public.catalog_list_tires_v1(text, text[], numeric, numeric, numeric, text, text, boolean, boolean, boolean, boolean, boolean, boolean, boolean, text, integer, integer) from public;
