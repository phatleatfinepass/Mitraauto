-- Phase 2: add tire segment filtering to the CMS tire catalog view settings.

drop function if exists public.cms_count_tires_admin_v1(text, boolean, boolean, text, text[], boolean, boolean, text[]);
drop function if exists public.cms_count_tires_admin_v1(text, boolean, boolean, text, text, text[], boolean, boolean, text[]);
...
drop function if exists public.cms_list_tires_admin_v1(text, boolean, boolean, text, text, text[], boolean, boolean, text[], integer, integer);
...
as $$
  ...
  left join public.catalog_selected_items selected_item
    on selected_item.id = mv.variant_id
  where public.catalog_selected_tire_cms_matches_search(mv, p_search)
    ...
    and (p_tire_segment is null or btrim(p_tire_segment) = '' or p_tire_segment = 'all' or selected_item.tire_segment = p_tire_segment)
...
as $$
  select
    ...
    mv.is_non_passenger,
    coalesce(selected_item.sound_absorber, false) as sound_absorber,
    mv.cms_data
  from public.catalog_selected_tires_cms_admin_v1 mv
  left join public.catalog_selected_items selected_item
    on selected_item.id = mv.variant_id
...
revoke all on function public.cms_count_tires_admin_v1(text, boolean, boolean, text, text, text[], boolean, boolean, text[]) from public;
grant execute on function public.cms_count_tires_admin_v1(text, boolean, boolean, text, text, text[], boolean, boolean, text[]) to authenticated;

revoke all on function public.cms_list_tires_admin_v1(text, boolean, boolean, text, text, text[], boolean, boolean, text[], integer, integer) from public;
grant execute on function public.cms_list_tires_admin_v1(text, boolean, boolean, text, text, text[], boolean, boolean, text[], integer, integer) to authenticated;
