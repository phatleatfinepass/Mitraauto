...
const DEFAULT_PAGE_SIZE = Number(Deno.env.get("RAW_RD_RIM_PAGE_SIZE") ?? 10000);
...
function normalizeBoltPattern(
  boltCount: number | null,
  boltCircle: number | null,
): string | null {
  if (boltCount === null || boltCircle === null) return null;
  return `${boltCount}x${formatDecimal(boltCircle)}`;
}

function materialFromGroup(value: unknown): string | null {
  const group = String(value ?? "").toLowerCase();
  if (
    group.includes("steel") || group.includes("pelti") ||
    group.includes("teräs")
  ) return "steel";
  if (
    group.includes("alloy") || group.includes("alu") ||
    group.includes("kevytmetalli")
  ) return "alloy";
  return null;
}

function imageUrlFromId(imageId: string | null): string | null {
  if (!imageId) return null;
  return `https://api.rengasduo.fi/api/images/${encodeURIComponent(imageId)}`;
}
...
function parseDeliveryDays(
  ...
  const numbers = Array.from(text.matchAll(/\d+/g)).map((match) =>
    Number(match[0])
  );
...
async function fetchRDRims(token: string): Promise<Record<string, unknown>[]> {
  if (!RD_BASE_URL) throw { where: "env", message: "RD_BASE_URL missing" };
  const base = RD_BASE_URL.endsWith("/")
    ? RD_BASE_URL.slice(0, -1)
    : RD_BASE_URL;
...
async function upsertInChunks(sb: any, rows: Record<string, unknown>[]) {
  for (let index = 0; index < rows.length; index += UPSERT_BATCH_SIZE) {
    ...
    const { error } = await sb
      .from("supplier_raw_rd_rims")
      .upsert(chunk, { onConflict: "external_id" });
    if (error) {
      throw {
        where: "upsert:supplier_raw_rd_rims",
...
async function markUnavailableBefore(
  sb: any,
...
async function successfulPagesForBatch(
  sb: any,
  batchKey: string,
): Promise<Set<number>> {
  const { data, error } = await sb
...
serve(async (req) => {
  ...
  const pageSize = Math.max(
    1,
    Number(requestUrl.searchParams.get("page_size") ?? DEFAULT_PAGE_SIZE) ||
      DEFAULT_PAGE_SIZE,
  );
  const finalize = requestUrl.searchParams.get("finalize") === "true" ||
    !pageParam;
  const batchKey = requestUrl.searchParams.get("batch_key") ??
    new Date().toISOString().slice(0, 10);
  const batchDate = batchKey.match(/^\d{4}-\d{2}-\d{2}/)?.[0] ??
    new Date().toISOString().slice(0, 10);
  ...
  try {
    ...
    const articles = await fetchRDRims(token);
    const totalPages = Math.max(1, Math.ceil(articles.length / pageSize));
    const sliceStart = (page - 1) * pageSize;
    const pageArticles = articles.slice(sliceStart, sliceStart + pageSize);
    ...
    for (const article of pageArticles) {
      ...
      const boltCount = toInteger(article.NumberOfBolts);
      const boltCircle = toNumber(article.BoltCircle);
      const imageId = toText(article.ImageId);
      const deliveryDays = parseDeliveryDays(article.ExternalDeliveryTime);
      ...
      rows.push({
        ...
        article_text: toText(article.ArticleText ?? article.ArticleName),
        size_text: toText(article.ArticleText),
        width_in: toNumber(article.Width),
        rim_diameter_in: toNumber(article.Diameter),
        bolt_count: boltCount,
        bolt_circle: boltCircle,
        bolt_pattern: normalizeBoltPattern(boltCount, boltCircle),
        et_offset_mm: toNumber(article.Offset),
        center_bore_mm: toNumber(article.CenterBore),
        color: toText(article.Color),
        finish: toText(article.Finish),
        material: materialFromGroup(article.MainGroupName),
        seat_type: toText(article.SeatType),
        wheel_load_kg: toNumber(article.MaximumLoad),
        stock_qty: toInteger(article.QuantityAvailable),
        external_stock_qty: toInteger(article.QuantityExternal),
        wholesale_price_eur: toNumber(article.NetPrice ?? article.Price),
        consumer_price_eur: toNumber(article.Price),
        retail_price_eur: toNumber(article.RetailPrice),
        delivery_days_min: deliveryDays.min,
        delivery_days_max: deliveryDays.max,
        image_id: imageId,
        image_url: imageUrlFromId(imageId),
        supplier_metadata_json: {
          main_group_name: toText(article.MainGroupName),
          article_name: toText(article.ArticleName),
          external_delivery_time: toText(article.ExternalDeliveryTime),
        },
        raw_payload: article,
    ...
    if (finalize) {
      ...
      if (missingPages.length === 0) {
        markedUnavailable = await markUnavailableBefore(
          sb,
          batchStartIso,
