...
function CatalogCMSPage() {
  ...
  const catalogTabs = [
    {
      ...
      label: 'Tires',
      description: 'Complete tire product readiness, pricing, media, labels, and publish checks.',
      module: 'catalog_tires',
    },
    {
      id: 'rims-refactor' as const,
      label: 'Rims',
      description: 'Operate the refactored rim lifecycle from selected winner layer to webshop publish.',
      module: 'catalog_rims',
    },
  ];
  ...
  if (!resolvedActiveTab) {
    return (
      <div className="space-y-2 p-8 text-muted-foreground">
        <h2 className="text-xl font-semibold text-foreground">Catalog access unavailable</h2>
        <p>Your CMS role does not include tire or rim catalog access.</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col gap-4 border-b bg-card px-6 py-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-foreground">
            Catalog - {activeCatalogTab?.label ?? 'Tires'}
          </h2>
          <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
            {activeCatalogTab?.description ?? 'Finalize tire and rim catalog readiness from one CMS workspace.'}
          </p>
        </div>
