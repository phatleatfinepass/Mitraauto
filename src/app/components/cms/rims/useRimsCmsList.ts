    ...
    try {
      ...
      if (rowsError && isRecoverableFetchError(rowsError)) {
        try {
          ...
          setTotalCount(fallbackRows.length === pageSize ? offset + fallbackRows.length + 1 : offset + fallbackRows.length);
          return;
        } catch (fallbackError: any) {
          if (isRecoverableFetchError(fallbackError) && rims.length > 0) {
      ...
      if (isRecoverableFetchError(err) && rims.length > 0) {
