    ...
    <article className={`rounded-2xl border p-4 shadow-[0_8px_24px_rgba(0,0,0,0.16)] ${toneClasses(item.tone)}`}>
      ...
      >
        <div className="min-w-0">
          ...
          {bookingCompact ? (
            <>
              <div className="mt-2 grid w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-x-2 text-sm text-white/72">
                <span className="inline-flex min-w-0 items-center gap-1.5">
                  <IdCard className="h-3.5 w-3.5 flex-none" />
                  <span className="truncate font-mono">{item.licensePlate}</span>
                </span>
                <span aria-hidden="true" />
                <span className="flex-none text-right text-xs text-white/55">{item.time}</span>
              </div>
              <div className="mt-2 grid w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-x-2 text-xs text-white/58">
                ...
                <span className="min-w-0 truncate px-2 text-center">{item.owner}</span>
                <span className="flex-none pl-2 text-right">{item.phone}</span>
              </div>
            </>
          ) : (
            <>
