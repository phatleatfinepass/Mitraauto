...
export function RimsCMSPage() {
  ...
  const [selectedRim, setSelectedRim] = useState<RimRow | null>(
    null,
  );
  const [cmsData, setCmsData] =
    useState<RimCMSData>(emptyCmsState);
  ...
  const fetchRims = async () => {
    ...
    const variantIds = (data ?? []).map(
      (item) => item.variant_id,
    );
    const cmsStatuses: Record<
      string,
      "Default" | "Overridden" | "Hidden"
    > = {};

    if (variantIds.length > 0) {
      const { data: cmsRows, error: cmsFetchError } =
        await supabase
          .from("product_cms")
      ...
      if (cmsFetchError) {
        setListError(
          "Failed to load CMS statuses: " +
            cmsFetchError.message,
        );
      } else if (cmsRows) {
        cmsRows.forEach((row) => {
          cmsStatuses[row.variant_id] = row.is_hidden
            ? "Hidden"
            : "Overridden";
        });
      }
    ...
    const mappedRims: RimRow[] = (data ?? []).map((item) => ({
      ...
      brand: item.brand ?? "",
      brand_display_name:
        item.brand_display_name ?? item.brand ?? "",
      model: item.model ?? "",
    ...
    if (
      selectedRim &&
      !mappedRims.find(
        (rim) => rim.variant_id === selectedRim.variant_id,
      )
    ) {
      setSelectedRim(null);
  ...
  const loadCmsData = async (variantId: string) => {
    ...
    if (data) {
      ...
      setCmsData({
        ...
        galleryUrls: Array.isArray(data.gallery)
          ? data.gallery.join(", ")
          : "",
  ...
  const handleSave = async () => {
    ...
    setHasCmsEntry(true);
    await Promise.all([
      loadCmsData(selectedRim.variant_id),
      fetchRims(),
    ]);
  ...
  return (
    <div
      className={`min-h-screen ${isDark ? "bg-[#0B0D10]" : "bg-gray-50"}`}
    >
      {/* Page Header */}
      <div
        className={`border-b ${isDark ? "bg-[#161A22] border-white/10" : "bg-white border-gray-200"}`}
      >
        <div className="px-8 py-6">
          ...
          >
            Manage rim data, override display text, images, and
            visibility
          </p>
        </div>
      </div>
      ...
      <div className="flex h-[calc(100vh-140px)]">
        ...
        <div
          className={`w-1/2 border-r ${isDark ? "border-white/10" : "border-gray-200"} flex flex-col`}
        >
          {/* Filters Panel */}
          <div
            className={`p-6 border-b ${isDark ? "bg-[#161A22] border-white/10" : "bg-white border-gray-200"}`}
          >
            <h2
            ...
            >
              Filters work on normalized rim data from
              products_search.
            </p>
          </div>
          ...
          <div className="flex-1 overflow-auto">
            <table className="w-full">
              <thead
                className={`sticky top-0 z-10 ${isDark ? "bg-[#1a1f2e]" : "bg-gray-100"}`}
              >
                <tr
                  className={`text-xs uppercase tracking-wider ${isDark ? "text-gray-400" : "text-gray-600"}`}
                >
              ...
              <tbody>
                {rims.length === 0 ? (
                  <tr>
                    <td
                      colSpan={11}
                      className={`px-4 py-8 text-center ${isDark ? "text-gray-500" : "text-gray-500"}`}
                    >
                  ...
                  rims.map((rim) => (
                    ...
                    >
                      ...
                      <td
                        className={`px-4 py-3 ${isDark ? "text-gray-400" : "text-gray-600"}`}
                      >
                        {rim.width_in ?? "-"}"
                      </td>
                      <td
                      ...
                      >
                        {rim.price
                          ? `${rim.currency ?? "€"}${rim.price.toFixed(2)}`
                          : "-"}
                      </td>
                      <td
                        className={`px-4 py-3 ${isDark ? "text-gray-400" : "text-gray-600"}`}
          ...
          {totalPages > 1 && (
            <div
              className={`px-4 py-3 border-t ${isDark ? "border-white/10 bg-[#161A22]" : "border-gray-200 bg-white"}`}
            >
        ...
        <div
          className={`w-1/2 flex flex-col ${isDark ? "bg-[#0B0D10]" : "bg-gray-50"}`}
        >
          <div className="flex-1 overflow-auto p-6">
            <h2
              className={`text-xl mb-6 ${isDark ? "text-white" : "text-gray-900"}`}
            >
              Rim CMS Overrides
            </h2>
            ...
            {!selectedRim ? (
              ...
              <>
                {/* Read-only Info Box */}
                <Card
                  className={`p-4 mb-6 ${isDark ? "bg-white/5 border-white/10" : "bg-gray-100 border-gray-200"}`}
                >
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      >
                        {selectedRim.brand_display_name}
                      </span>
                    </div>
                    <div>
                      <span
                    ...
                    <div>
                      ...
                      <span
                        className={`ml-2 ${isDark ? "text-white" : "text-gray-900"}`}
                      >
                        {selectedRim.width_in ?? "-"}" /{" "}
                        {selectedRim.rim_diameter_in ?? "-"}"
                      </span>
                    </div>
                    <div>
                      <span
                    ...
                    <div>
                      ...
                      <span
                        className={`ml-2 ${isDark ? "text-white" : "text-gray-900"}`}
                      >
                        {selectedRim.et_offset_mm !== null
                          ? `ET${selectedRim.et_offset_mm}`
                          : "-"}
                      </span>
                    </div>
                    <div>
                      ...
                      <span
                        className={`ml-2 ${isDark ? "text-white" : "text-gray-900"}`}
                      >
                        {[selectedRim.color, selectedRim.finish]
                          .filter(Boolean)
                          .join(" / ") || "-"}
                      </span>
                    </div>
                    <div className="col-span-2">
                      ...
                      <span
                        className={`ml-2 ${isDark ? "text-white" : "text-gray-900"}`}
                      >
                        {selectedRim.card_title ||
                          `${selectedRim.brand_display_name} ${selectedRim.model}`}
                      </span>
                    </div>
                    <div className="col-span-2">
                      <span
                  ...
                  <div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        >
                          If checked, this rim will not appear
                          in search or listing pages. Hiding is
                          applied via the CMS layer.
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          {selectedRim && (
            <div
              className={`p-6 border-t ${isDark ? "bg-[#161A22] border-white/10" : "bg-white border-gray-200"}`}
            >
